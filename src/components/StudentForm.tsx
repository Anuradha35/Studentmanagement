...
useEffect(() => {
 ...
 // 🔄 MODIFY the handleDuplicateConfirmation function - REPLACE the existing one with this:
...
<div className="mt-1 pt-3 border-t border-gray-700 text-sm">
  ...
  <button 
    type="button"
    onClick={() => {
      console.log("🔥 ENHANCED Add to Current Group button clicked");
  ...
  if (currentStudentCheck.hasAlreadyPaid) {
    ...
    return;
      ...
      console.log("🔍 Current father name:", currentFatherName);
      ...
      // ✅ CRITICAL CHECK 1: Is student member of existing group?
      ...
      if (!isStudentInGroup) {
        // Student is NOT in group - REJECT
        ...
        clearAllFieldsAndReset();
        return;
      }
      
      // ✅ CRITICAL CHECK 2: Has this student already paid with this UTR/Receipt?
      console.log("🔍 Checking if student has already paid...");
      
      // Get payment details from duplicateInfo
      const paymentUTR = duplicateInfo.existingPayment.utrId || '';
      const paymentReceipt = duplicateInfo.existingPayment.receiptNo || '';
      const paymentDate = duplicateInfo.existingPayment.paymentDate || '';
      
      console.log("🔍 Payment UTR:", paymentUTR);
      console.log("🔍 Payment Receipt:", paymentReceipt);
      console.log("🔍 Payment Date:", paymentDate);
      
      // Check if this student has already used this payment method
      // This requires checking existing database records - assuming you have access to payment history
      const hasStudentAlreadyPaid = checkIfStudentAlreadyPaid(
        currentStudentName,
        paymentUTR,
        paymentReceipt,
        paymentDate
      ...
      if (hasStudentAlreadyPaid) {
        // STUDENT HAS ALREADY PAID - PREVENT DUPLICATE
        console.log("❌ DUPLICATE PAYMENT DETECTED!");
        
        setTimeout(() => {
          const paymentMethod = paymentUTR ? 'UTR/UPI ID' : 'Receipt Number';
          const paymentValue = paymentUTR || paymentReceipt;
          ...
          alert(`❌ DUPLICATE PAYMENT DETECTED!\n\n` +
                ...
                `❌ This student has ALREADY PAID using this ${paymentMethod}!\n\n` +
                `You cannot use the same payment method twice for the same student.\n` +
                `Each student can only pay once per payment transaction.\n\n` +
                `✅ Please use a different ${paymentMethod} for new enrollment.`);
        }, 100);
      ...
      // ✅ CRITICAL CHECK 3: Course validation for same student
      ...
      if (isSameCourse && isSameBatch && isSameYear && isSameDuration) {
        // EXACT SAME COURSE - This is suspicious, might be duplicate enrollment
        console.log("⚠️ EXACT SAME COURSE DETECTED");
        
        setTimeout(() => {
          const confirmMsg = `⚠️ SAME COURSE ENROLLMENT DETECTED!\n\n` +
                           `Student: ${formData.studentName}\n` +
                           `Course: ${selectedCourse}\n` +
                           `Batch: ${selectedBatch}\n` +
                           `Year: ${selectedYear}\n` +
                           `Duration: ${formData.courseDuration} months\n\n` +
                           `⚠️ This appears to be the same course enrollment.\n` +
                           `Are you sure this is a new payment and not a duplicate entry?\n\n` +
                           `Click OK only if you're certain this is a different payment for the same course.`;
          
          if (!confirm(confirmMsg)) {
            console.log("🚫 User cancelled same course confirmation");
            clearAllFieldsAndReset();
            return;
          }
          
          // User confirmed - proceed
          proceedWithGroupAddition();
        }, 100);
        
        return;
      }
      
      // ✅ DIFFERENT COURSE - Show warning but allow
      if (!isSameCourse || !isSameBatch || !isSameYear) {
        console.log("⚠️ DIFFERENT COURSE/BATCH/YEAR DETECTED");
        
        setTimeout(() => {
          const warningMsg = `⚠️ DIFFERENT COURSE DETAILS DETECTED!\n\n` +
                           `CURRENT ENTRY:\n` +
                           `- Course: ${selectedCourse}\n` +
                           `- Batch: ${selectedBatch}\n` +
                           `- Year: ${selectedYear}\n` +
                           ...
                           `Student "${formData.studentName}" is being enrolled in a different course/batch using the same payment.\n\n` +
                           `Do you want to proceed?`;
          
          if (!confirm(warningMsg)) {
            console.log("🚫 User cancelled different course warning");
            clearAllFieldsAndReset();
            return;
          }
      ...
      // If we reach here, proceed directly
      proceedWithGroupAddition();
      
      // ✅ HELPER FUNCTION: Check if student already paid
      function checkIfStudentAlreadyPaid(studentName, utrId, receiptNo, paymentDate) {
        // This function should check your database/payment records
        // For now, I'll provide the logic structure
        
        console.log("🔍 Checking payment history for:", {
          studentName,
          utrId,
          receiptNo,
          paymentDate
        ...
        // Example logic:
        /*
        const existingPayments = getStudentPaymentHistory(studentName);
        
        return existingPayments.some(payment => 
          (payment.utrId && payment.utrId === utrId) ||
          (payment.receiptNo && payment.receiptNo === receiptNo) ||
          (payment.paymentDate === paymentDate && 
           payment.studentName.toUpperCase() === studentName)
        );
        */
        
        // For now, return false to allow testing
        // You should implement actual database check here
        return false;
      }
      
      // ✅ PROCEED WITH GROUP ADDITION
      function proceedWithGroupAddition() {
        console.log("✅ All validations passed - proceeding with group addition");
      ...
      // ✅ PRE-FILLING LOGIC (Same as before)
      function proceedWithPreFilling() {
        try {
          ...
          const newGroupEntries = existingStudentNames.map((studentName, index) => ({
            studentName: studentName.charAt(0) + studentName.slice(1).toLowerCase(),
            amount: '',
            onlineAmount: '',
            offlineAmount: '',
            utrId: '',
          ...
          setTimeout(() => {
            ...
            const successMsg = `✅ Payment details pre-filled successfully!\n\n` +
      ...
      // ✅ COMPREHENSIVE CLEAR AND RESET FUNCTION
      function clearAllFieldsAndReset() {
        console.log("🧹 COMPREHENSIVE RESET: Clearing all fields and resetting to fresh state");
        
        // Close modal and clear duplicate info
        setDuplicateCheckModal(false);
        setDuplicateInfo(null);
        ...
        setSingleOnlineAmount('');
        setSingleOfflineAmount('');
        setSingleUtrId('');
        setSingleReceiptNo('');
        ...
        console.log("✅ Complete reset successful - ready for fresh entry");
        
        // Focus on student name field after reset
        setTimeout(() => {
          const studentNameField = document.querySelector('input[name="studentName"]');
          if (studentNameField) {
            studentNameField.focus();
            console.log("✅ Cursor focused on student name field");
          }
        }, 100);
      }
