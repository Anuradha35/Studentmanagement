...
const StudentForm: React.FC<StudentFormProps> = ({
  ...
  const [duplicateInfo, setDuplicateInfo] = useState<{
    ...
    value: string;
    existingPayment: any;
    studentInfo?: any; // ✅ SAFETY: Made optional
    courseName?: string; // ✅ SAFETY: Made optional
    batchName?: string; // ✅ SAFETY: Made optional
    yearName?: string; // ✅ SAFETY: Made optional
  ...
  const validateGroupEntries = () => {
    for (let i = 0; i < dynamicGroupEntries.length; i++) {
      const entry = dynamicGroupEntries[i];
      if (!entry || !entry.studentName?.trim()) {
        alert(`Student Name #${i + 1} cannot be blank`);
        const element = document.getElementById(`studentName-${i}`);
    ...
    for (const [yearKey, yearData] of Object.entries(appData.years || {})) {
      for (const [courseKey, courseData] of Object.entries(yearData || {})) {
        for (const [batchKey, batchData] of Object.entries(courseData || {})) {
          // ✅ SAFETY: Check if students array exists and is valid
          if (!batchData || !Array.isArray(batchData.students)) continue;

          for (const student of batchData.students) {
            // ✅ SAFETY: Check if student exists
            if (!student || !student.id) continue;

            // Check payments for this student
            const studentPayments = appData.payments?.filter(p => p && p.studentId === student.id) || [];
            
            for (const payment of studentPayments) {
              // ✅ SAFETY: Check if payment exists
              if (!payment) continue;
              ...
              if (isMatch) {
                ...
                if (!mainPayment) {
                  mainPayment = {
                    ...
                    studentInfo: student, // ✅ SAFETY: Store student info here
                ...
                if (payment.type === 'group' && payment.groupId) {
                  ...
                  for (const groupPayment of sameGroupPayments) {
                    if (!groupPayment) continue; // ✅ SAFETY: Check payment exists
                    ...
                    for (const [y, yData] of Object.entries(appData.years || {})) {
                      for (const [c, cData] of Object.entries(yData || {})) {
                        for (const [b, bData] of Object.entries(cData || {})) {
                          // ✅ SAFETY: Check if students array exists
                          const students = Array.isArray(bData?.students) ? bData.students : [];
                          const groupStudent = students.find(s => s && s.id === groupPayment.studentId);

                          if (groupStudent) {
                            // Check if this student is not already added
                            const alreadyAdded = allGroupMembers.some(member => 
                              member.studentInfo && member.studentInfo.id === groupStudent.id
                            );
    ...
    for (const yearKey in appData.years || {}) {
      for (const courseKey in appData.years[yearKey] || {}) {
        for (const batchKey in appData.years[yearKey][courseKey] || {}) {
          ...
          if (!batch || !Array.isArray(batch.students)) continue;
  ...
  // ✅ ENHANCED SAFETY: Safe setter for dynamic group entries
  ...
  useEffect(() => {
    ...
    payments.forEach(payment => {
      if (payment && payment.paymentMode === 'online' && payment.utrId) {
        newExistingPayments.utrIds.add(payment.utrId);
      }
      if (payment && payment.paymentMode === 'offline' && payment.receiptNo) {
  ...
  // ✅ ENHANCED SAFETY: Safer access to dynamicGroupEntries
  useEffect(() => {
    if (dynamicGroupEntries.length > 0 && formData.studentName) {
      const updated = [...dynamicGroupEntries];
      // ✅ SAFETY: Check if first entry exists before updating
      if (updated[0]) {
  ...
  // ✅ ENHANCED SAFETY: More robust group entries sync
  useEffect(() => {
  ...
  const handleDuplicateConfirmation = (action: 'proceed' | 'cancel') => {
    ...
    if (action === 'proceed' && duplicateInfo.paymentType === 'group' && paymentType === 'group') {
      ...
      if (mainMember?.studentInfo && mainMember?.existingPayment) {
        ...
        setFormData(prev => ({
          ...prev,
          studentName: existingStudent.studentName || prev.studentName,
          fatherName: existingStudent.fatherName || prev.fatherName,
          gender: existingStudent.gender || prev.gender,
          mobileNo: existingStudent.mobileNo || prev.mobileNo,
          email: existingStudent.email || prev.email,
          category: existingStudent.category || prev.category,
          hostler: existingStudent.hostler || prev.hostler,
          collegeName: existingStudent.collegeName || prev.collegeName,
        ...
        const allGroupNames = duplicateInfo.allGroupMembers?.map(m => m?.studentInfo?.studentName || 'Unknown') || [];
        ...
        safeSetDynamicGroupEntries(newGroupEntries);
  ...
  const handlePaymentTypeChange = (newPaymentType) => {
    if (paymentType !== newPaymentType) {
      ...
      if (paymentType === 'single' && newPaymentType === 'group') {
        ...
        safeSetDynamicGroupEntries(initialGroupEntries);
        setGroupCount(2);
        ...
        if (resetEntries.length > 0 && resetEntries[0]) {
        ...
        safeSetDynamicGroupEntries(resetEntries);
  ...
  const handleGroupCountConfirm = () => {
    ...
    safeSetDynamicGroupEntries(entries);
    setGroupCount(count);
  ...
  const handleAddGroupPayment = () => {
    ...
    // ✅ SAFETY: Check if first entry exists before accessing amount
        ...
        return Object.values(appData.years || {}).some((courses) =>
          Object.values(courses || {}).some((batches) =>
            Object.values(batches || {}).some((batch) =>
              Array.isArray(batch?.students) && batch.students.some((s) =>
                Array.isArray(s?.payments) && s.payments.some(
                  (p) =>
                ...
                <div className="mb-6 p-4 bg-blue-500/20 border border-blue-500/30 rounded-lg">
                  ...
                  <div className="text-right mb-2">
                    <span className="text-2xl font-bold text-white">
                      Total: ₹
                      {groupPayments.reduce((sum, p) => sum + (p?.total || 0), 0).toLocaleString()}
                    </span>
                  </div>
                    ...
                    {groupPayments.map((payment, index) => (
                      <div key={index}>
                        ...
                        <div className="flex justify-between items-center text-blue-200">
                          <span>{payment?.studentName || 'Unknown'}</span>
                          <span>₹{(payment?.student1Amount || 0).toLocaleString()}</span>
                        </div>
                        ...
                        {payment?.otherStudents && (
                          <div className="flex justify-between items-center text-blue-200">
                            ...
                            <span>₹{(payment?.otherStudentsAmount || 0).toLocaleString()}</span>
                          </div>
                        )}
                      </div>
                    ))}
                    ...
                    <div key={index} className="p-3 bg-slate-700 rounded-lg">
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-white font-medium">
                            {(dynamicGroupEntries || []).map(s => s?.studentName || 'Unknown').join(', ')}
                          </p>
                          <p className="text-gray-400 text-sm">
                            {(payment?.onlineAmount || 0) > 0 && `Online: ₹${(payment.onlineAmount || 0)} (UTR: ${payment.utrId || 'N/A'})`}
                            {(payment?.onlineAmount || 0) > 0 && (payment?.offlineAmount || 0) > 0 && ' | '}
                            {(payment?.offlineAmount || 0) > 0 && `Offline: ₹${(payment.offlineAmount || 0)} (Receipt: ${payment.receiptNo || 'N/A'})`}
                          </p>
                          <p className="text-gray-400 text-sm">Date: {payment?.paymentDate || 'N/A'}</p>
                          <p className="text-green-400 text-sm font-medium">
                            Total: ₹{((payment?.onlineAmount || 0) + (payment?.offlineAmount || 0)).toLocaleString()}
                          </p>
                        </div>
                        <button
                          type="button"
          ...
          {duplicateInfo && (
            <div className="space-y-6 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-3">
                    <div>
                      ...
                      <p className="text-white font-medium">{duplicateInfo.studentInfo?.studentName || 'Unknown'}</p>
                    </div>
                    <div>
                      <p className="text-gray-400 text-sm">Father's Name</p>
                      <p className="text-white">{duplicateInfo.studentInfo?.fatherName || 'Unknown'}</p>
                    </div>
                    <div>
                      <p className="text-gray-400 text-sm">Mobile Number</p>
                      <p className="text-white">{duplicateInfo.studentInfo?.mobileNo || 'Unknown'}</p>
                    </div>
                    <div>
                      <p className="text-gray-400 text-sm">Email</p>
                      <p className="text-white text-sm">{duplicateInfo.studentInfo?.email || 'Unknown'}</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <p className="text-white font-medium">{duplicateInfo.courseName || 'Unknown'}</p>
                    </div>
                    <div>
                      <p className="text-gray-400 text-sm">Batch & Year</p>
                      <p className="text-white">{duplicateInfo.batchName || 'Unknown'} • {duplicateInfo.yearName || 'Unknown'}</p>
                    </div>
                    <div>
                      <p className="text-gray-400 text-sm">Duration</p>
                      <p className="text-white">{duplicateInfo.studentInfo?.courseDuration || 'Unknown'}</p>
                    </div>
                    <div>
                      <p className="text-gray-400 text-sm">Course Dates</p>
                      <p className="text-white">{duplicateInfo.studentInfo?.startDate || 'Unknown'} to {duplicateInfo.studentInfo?.endDate || 'Unknown'}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-blue-500/20 rounded-lg p-4 border border-blue-500/30">
                  <p className="text-blue-300 text-sm">Course Fee</p>
                  <p className="text-2xl font-bold text-white">₹{duplicateInfo.studentInfo?.courseFee?.toLocaleString() || 0}</p>
                </div>
                <div className="bg-green-500/20 rounded-lg p-4 border border-green-500/30">
                  <p className="text-green-300 text-sm">Total Paid</p>
                  <p className="text-2xl font-bold text-white">₹{duplicateInfo.studentInfo?.totalPaid?.toLocaleString() || 0}</p>
                </div>
                <div className="bg-orange-500/20 rounded-lg p-4 border border-orange-500/30">
                  <p className="text-orange-300 text-sm">Remaining</p>
                  <p className="text-2xl font-bold text-white">₹{duplicateInfo.studentInfo?.remainingFee?.toLocaleString() || 0}</p>
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <div>
                        ...
                        <p className="text-white">{duplicateInfo.existingPayment?.paymentDate || 'Unknown'}</p>
                      </div>
                      <div>
                        <p className="text-gray-400 text-sm">Payment Mode</p>
                        <p className="text-white">
                          {(duplicateInfo.existingPayment?.onlineAmount || 0) > 0 && (duplicateInfo.existingPayment?.offlineAmount || 0) > 0
                            ? '💳 Online + 💵 Offline'
                            : (duplicateInfo.existingPayment?.onlineAmount || 0) > 0
                            ? '💳 Online'
                            : '💵 Offline'
                          }
                        </p>
                      ...
                      {duplicateInfo.paymentType === 'single' ? (
                        <div>
                          ...
                          <p className="text-2xl font-bold text-green-400">
                            ₹{duplicateInfo.existingPayment?.amount?.toLocaleString() || 0}
                          </p>
                        </div>
                      ) : (
                        <div className="space-y-2">
                          <div>
                            ...
                            <p className="text-xl font-bold text-purple-400">
                              ₹{duplicateInfo.existingPayment?.totalGroupAmount?.toLocaleString() || 0}
                            </p>
                          </div>
                          <div className="grid grid-cols-2 gap-2 text-sm">
                            ...
                            <div>
                              ...
                              <p className="text-blue-400 font-medium">
                                ₹{((duplicateInfo.existingPayment?.totalGroupAmount || 0) - (duplicateInfo.existingPayment?.amount || 0))?.toLocaleString() || 0}
                              </p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="text-sm space-y-1 mt-2 pt-2 border-t border-gray-600">
                          {(duplicateInfo.existingPayment?.onlineAmount || 0) > 0 && (
                            <div className="flex justify-between">
                              ...
                              <span className="text-white">₹{duplicateInfo.existingPayment.onlineAmount?.toLocaleString() || 0}</span>
                            </div>
                          )}
                          {(duplicateInfo.existingPayment?.offlineAmount || 0) > 0 && (
                            <div className="flex justify-between">
                              <span className="text-gray-400">💵 Offline Amount:</span>
                              <span className="text-white">₹{duplicateInfo.existingPayment.offlineAmount?.toLocaleString() || 0}</span>
                            </div>
                          )}
              ...
              {duplicateInfo.paymentType === 'single' || paymentType === 'single' ? (
                ...
                <div className="text-blue-300 text-sm bg-blue-500/10 border border-blue-500/30 rounded-lg p-4">
                  ...
                  <p>Since both payments are group payments, you can add <strong>{duplicateInfo.studentInfo?.studentName || 'the student'}</strong> to your current group as Student #1. 
            ...
            {duplicateInfo?.paymentType === 'group' && paymentType === 'group' && (
              <button 
                ...
                onClick={() => {
                  ...
                  // Enhanced safety with proper null checks
                  const currentStudentName = formData.studentName.trim().toUpperCase();
                  const currentFatherName = formData.fatherName.trim().toUpperCase();
                  ...
                  function proceedWithPreFilling() {
                    try {
                      ...
                      if (isSameCourse && isSameBatch && isSameYear && isSameDuration) {
                        ...
                        setFormData(prev => ({
                          ...prev,
                          fatherName: duplicateInfo.studentInfo?.fatherName || prev.fatherName,
                          gender: duplicateInfo.studentInfo?.gender || prev.gender,
                          mobileNo: duplicateInfo.studentInfo?.mobileNo || prev.mobileNo,
                          email: duplicateInfo.studentInfo?.email || prev.email,
                          category: duplicateInfo.studentInfo?.category || prev.category,
                          hostler: duplicateInfo.studentInfo?.hostler || prev.hostler,
                          collegeName: duplicateInfo.studentInfo?.collegeName || prev.collegeName,
                          branch: duplicateInfo.studentInfo?.branch || prev.branch,
                          startDate: duplicateInfo.studentInfo?.startDate || prev.startDate,
                          endDate: duplicateInfo.studentInfo?.endDate || prev.endDate
                        }));
                      }
                      
                      // ✅ Pre-fill payment information
                      if ((existingPayment?.onlineAmount || 0) > 0) {
                        setGroupOnlineAmount((existingPayment.onlineAmount || 0).toString());
                        setGroupUtrId(existingPayment.utrId || '');
                        console.log("✅ Pre-filled online payment:", existingPayment.onlineAmount, existingPayment.utrId);
                      }
                      
                      if ((existingPayment?.offlineAmount || 0) > 0) {
                        setGroupOfflineAmount((existingPayment.offlineAmount || 0).toString());
                        setGroupReceiptNo(existingPayment.receiptNo || '');
